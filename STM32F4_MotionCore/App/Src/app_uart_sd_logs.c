...
{
	APP_STATE_Update_Error_BeforeRTOSStart(ERR_SD_CARD_LOG, 0);
	APP_STATE_Update_Error_BeforeRTOSStart(ERR_UART_LOG, 1);
...
static void uart_sd_logging_task(void* pvParameters)
{
	...
	/* SD configuration before use */
	{
		...
		if(sd_state == FR_OK)
		{
			APP_STATE_Update_Error(ERR_SD_CARD_LOG, 0);
			/* Create new work directory */
			sd_state = f_mkdir(SD_WORK_DIR_PATH);
			if(sd_state != FR_OK)
			{
				APP_STATE_Update_Error(ERR_SD_CARD_LOG, 1);
			}
			else
			{
				APP_STATE_Update_Error(ERR_SD_CARD_LOG, 0);
		...
		else APP_STATE_Update_Error(ERR_SD_CARD_LOG, 1);
	...
	while(1)
	{
		...
		/* UART management */
		{
			...
			{
				APP_STATE_Update_Error(ERR_UART_LOG, 1);
			}
			else
			{
				APP_STATE_Update_Error(ERR_UART_LOG, 0);
			}
		}
