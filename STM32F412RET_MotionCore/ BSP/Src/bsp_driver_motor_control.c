...
static void BSP_MOTOR_State(float pwm_percent, uint8_t MOTOR_STATE)
{
	...
	if(MOTOR_STATE == MOTOR_FORWARD)
	{
		...
		__HAL_TIM_SET_COMPARE(local_ptim1, TIM_CHANNEL_2, 0U);
	}
	else if(MOTOR_STATE == MOTOR_BACKWARD)
	{
		__HAL_TIM_SET_COMPARE(local_ptim1, TIM_CHANNEL_1, 0U);
		__HAL_TIM_SET_COMPARE(local_ptim1, TIM_CHANNEL_2, compare_value);
	}
	...
	{
		uint32_t arr = __HAL_TIM_GET_AUTORELOAD(local_ptim1);
		__HAL_TIM_SET_COMPARE(local_ptim1, TIM_CHANNEL_1, arr);
		__HAL_TIM_SET_COMPARE(local_ptim1, TIM_CHANNEL_2, arr);
	}
}
...
DevStatus_t BSP_MOTOR_CONTROL_Init(TIM_HandleTypeDef* ptim1, TIM_HandleTypeDef* ptim2)
{
	...
	if((ptim1 != NULL) && (ptim2 != NULL))
	{
		...
		HAL_GPIO_WritePin(MOTOR_GPIO_PORT, MOTOR_GPIO_PIN_FORWARD_EN, GPIO_PIN_SET);
		HAL_GPIO_WritePin(MOTOR_GPIO_PORT, MOTOR_GPIO_PIN_BACKWARD_EN, GPIO_PIN_SET);
		...
		ret = (DevStatus_t)HAL_TIM_PWM_Start(local_ptim1, TIM_CHANNEL_2);
		if(ret != DRV_OK)
		{
			return DRV_INIT_NEEDED;
		}
...
DevStatus_t BSP_MOTOR_ChangePWM_State(float pwm_percent, uint8_t MOTOR_STATE)
{
	if(local_ptim1 != NULL)
	{
		...
		BSP_MOTOR_State(pwm_percent, MOTOR_STATE);
	}
	else
	{
		return DRV_INIT_NEEDED;
