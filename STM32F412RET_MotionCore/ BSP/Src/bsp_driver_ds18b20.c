...
static UART_HandleTypeDef* local_puart2_DS18B20 = NULL;
...
{
	local_puart2_DS18B20->Instance = USART2;
	local_puart2_DS18B20->Init.BaudRate = baudrate;
	local_puart2_DS18B20->Init.WordLength = UART_WORDLENGTH_8B;
	local_puart2_DS18B20->Init.StopBits = UART_STOPBITS_1;
	local_puart2_DS18B20->Init.Parity = UART_PARITY_NONE;
	local_puart2_DS18B20->Init.Mode = UART_MODE_TX_RX;
	local_puart2_DS18B20->Init.HwFlowCtl = UART_HWCONTROL_NONE;
	local_puart2_DS18B20->Init.OverSampling = UART_OVERSAMPLING_16;
	if (HAL_HalfDuplex_Init(local_puart2_DS18B20) != HAL_OK)
	{
...
static DevStatus_t BSP_DS18B20_InitWrite(uint8_t data)
{
	...
	return (DevStatus_t)HAL_UART_Transmit(local_puart2_DS18B20, tx_buf, 8, 100);
}
...
static DS18B20_ReadStatus_t BSP_DS18B20_InitRead(void)
{
	...
	for(uint32_t i = 0; i < 8; ++i)
	{
		...
		ret.state = (DevStatus_t)HAL_UART_Transmit(local_puart2_DS18B20, &tx_init_byte, 1, 10);
		if(ret.state != DRV_OK)
		{
		...
		HAL_UART_Receive(local_puart2_DS18B20, &rx_init_buf[i], 1, 10);
		if(ret.state != DRV_OK)
		{
			ret.state = DRV_INIT_NEEDED;
			return ret;
...
DevStatus_t BSP_DS18B20_ResetPresence(void)
{
	...
	/* Reset */
	ret = (DevStatus_t)HAL_UART_Transmit(local_puart2_DS18B20, &data_for_reset_presence, 1, 100); // Low for 500+ ms
	if(ret != DRV_OK)
	{
		return ret;
	}
	...
	/* Presence */
	ret = (DevStatus_t)HAL_UART_Receive(local_puart2_DS18B20, &data_for_reset_presence, 1, 100);
...
DevStatus_t BSP_DS18B20_Write(uint8_t data)
{
	...
	ret = (DevStatus_t)HAL_UART_Receive_DMA(local_puart2_DS18B20, rx_buf_read, 8);
	if(ret != DRV_OK)
	{
		return ret;
	}

	return (DevStatus_t)HAL_UART_Transmit_DMA(local_puart2_DS18B20, tx_buf, 8);
}
...
DevStatus_t BSP_DS18B20_ReadStart(void)
{
	...
	return (DevStatus_t)HAL_UART_Transmit_DMA(local_puart2_DS18B20, tx_buf_read, 8);
}
...
DevStatus_t BSP_DS18B20_Init(UART_HandleTypeDef* huart2)
{
	DevStatus_t ret;
	uint8_t command;
