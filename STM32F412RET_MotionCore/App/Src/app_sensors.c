...
void APP_SENSORS_Init(UART_HandleTypeDef* phuart2, I2C_HandleTypeDef* phi2c1)
{
	APP_STATE_Update_Error_BeforeRTOSStart(ERR_TEMP_SENSOR, ERR_NOT_ACTIVE);
	APP_STATE_Update_Error_BeforeRTOSStart(ERR_POWER_SENSOR, ERR_NOT_ACTIVE);
	/* Set all peripheral handles in BSP */
	for(uint32_t i = 0; i < 3; ++i)
	{
		if(BSP_DS18B20_Init(phuart2) == DRV_OK){
			APP_STATE_Update_Error_BeforeRTOSStart(ERR_TEMP_SENSOR, ERR_NOT_ACTIVE);
			break;
		}
	}
...
static void uart_i2c_sensors_task(void* pvParameters)
{
	...
	{
		/* Get temperature */
		temperature = OSAL_DS18B20_GetTemperature();

		/* Get electricity */
		electricity = OSAL_INA219_GetElectricity();

		/* Check errors */
		if (temperature.state != DRV_OK)
		{
			APP_STATE_Update_Error(ERR_TEMP_SENSOR, ERR_NOT_ACTIVE);
		}
		else {
			APP_STATE_Update_Error(ERR_TEMP_SENSOR, ERR_NOT_ACTIVE);
		}
		...
		{
			APP_STATE_Update_Error(ERR_POWER_SENSOR, ERR_NOT_ACTIVE);
		}
		else {
			APP_STATE_Update_Error(ERR_POWER_SENSOR, ERR_NOT_ACTIVE);
		}
