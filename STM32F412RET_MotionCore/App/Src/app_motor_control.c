...
void APP_MOTOR_CONTROL_init(TIM_HandleTypeDef* ptim1, TIM_HandleTypeDef* ptim2)
{
	...
	xTaskCreate(motor_control_task, "motor_control_task", 1024, NULL, 3, &motor_control_handle);
	configASSERT(motor_control_handle != NULL);
}

static float pid_pwm_percent;
static float encoder_delta;
...
static void motor_control_task(void* pvParameters)
{
	...
	APP_STATE_Set_Motor_TargetSpeed(10.0f);
	...
	while(1)
	{
		   ...
		   (global_state.target_motor_speed != 0))
		{
			...
			PID_Controller.differentiator = 0.0f;
			PID_Controller.prev_error = 0.0f;
			PID_Controller.prev_measurement = actual_speed_percent;

			APP_STATE_Set_Motor_State(MOTOR_RUN);
		}

		encoder_delta = (float)OSAL_MOTOR_CONTROL_GetEncoderDifference();
		...
		// low-pass filter
		filtered_speed = SPEED_FILTER_ALPHA * actual_speed_percent
		               + (1.0f - SPEED_FILTER_ALPHA) * filtered_speed;

		/* Target PWM value from PID control system */
		pid_pwm_percent = APP_MOTOR_CONTROL_PID_Update(&PID_Controller, filtered_speed, global_state.target_motor_speed);
