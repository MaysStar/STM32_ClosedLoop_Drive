...
static void user_communication_task(void* pvParameters)
{
	...
	while(1)
	{
		...
		float temp_current = g_data_state.current_A * 1000.0f;
		if (temp_current < 0.0f) temp_current = 0.0f;
		if (temp_current > 4000000000.0f) temp_current = 4000000000.0f;

		APP_USER_COMMUNICATION_ConvertU32_to_U8(can_tx_buf[1].data, (uint32_t)temp_current);
